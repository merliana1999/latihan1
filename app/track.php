...
class track extends Data2 {
	...
	public function __construct()
	{
		...
		public function tambah(String $d1,$d2,$d3,$d4){
			$sql = "INSERT INTO tb_track VALUES (NULL, :nama, :idart, :idalb, :times)";
			$stmt = $this->db->prepare($sql);
		...
		{
			$sql = "SELECT * FROM tb_track WHERE track_id = :id";
			$stmt = $this->db->prepare($sql);
		...
		{
			$sql = "UPDATE tb_track SET artist_id = :aid, album_id = :alid, track_name = :name, time = :time WHERE track_id = :id";
			$stmt = $this->db->prepare($sql);
			$stmt -> bindParam(":id", $d1);
		...
		{
			$sql = "DELETE FROM tb_track WHERE track_id = :id";
			$stmt = $this->db->prepare($sql);
			$stmt -> bindParam(":id", $d1);
			$stmt->execute();
		}
